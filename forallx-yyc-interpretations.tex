...
Sometimes, we can reason about all interpretations fairly easily. For example, we can offer a relatively simple argument that `$\atom{R}{a,a}\eiff \atom{R}{a,a}$' is a validity:
	\begin{quote}
		\label{allmodels1}
		Any relevant interpretation will give `$\atom{R}{a,a}$' a truth value. If `$\atom{R}{a,a}$' is true in an interpretation, then `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in that interpretation. If `$\atom{R}{a,a}$' is false in an interpretation, then `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in that interpretation. These are the only alternatives. So `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in every interpretation. Therefore, it is a validity.
	\end{quote}
...
Here is another bit of reasoning. Consider the sentence `$\forall x(\atom{R}{x,x}\eiff \atom{R}{x,x})$'. Again, it should obviously be a validity, but to say precisely why is quite a challenge. We cannot say that `$\atom{R}{x,x} \eiff \atom{R}{x,x}$' is true in every interpretation, since `$\atom{R}{x,x} \eiff \atom{R}{x,x}$' is not even a \emph{sentence} of FOL (remember that `$x$' is a variable, not a name). So we have to be a bit cleverer. 
	\begin{quote}
		Consider some arbitrary interpretation. Consider some arbitrary member of the domain, which, for convenience, we will call \emph{obbie}, and suppose we extend our original interpretation by adding a new name, `$c$', to name \emph{obbie}. Then either `$\atom{R}{c,c}$' will be true or it will be false. If `$\atom{R}{c,c}$' is true, then `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' is true. If `$\atom{R}{c,c}$' is false, then `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' will be true. So either way, `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' is true. Since there was nothing special about \emph{obbie}---we might have chosen any object---we see that no matter how we extend our original interpretation by allowing `$c$' to name some new object, `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' will be true in the new interpretation. So `$\forall x (\atom{R}{x,x} \eiff \atom{R}{x,x})$' was true in the original interpretation. But we chose our interpretation arbitrarily, so `$\forall x (\atom{R}{x,x} \eiff \atom{R}{x,x})$' is true in every interpretation. It is therefore a validity.
	\end{quote}
...
After all, if everything is both $H$ and $J$, then everything is~$H$. But we can only show that the argument is valid by considering what must be true in every interpretation in which the premise is true. To show this, we would have to reason as follows:
	\begin{quote}
		Consider an arbitrary interpretation in which the premise `$\forall x(\atom{H}{x} \eand \atom{J}{x})$' is true. It follows that, however we expand the interpretation with a new name, for example `$c$', `$\atom{H}{c} \eand \atom{J}{c}$' will be true in this new interpretation. `$\atom{H}{c}$' will, then, also be true in this new interpretation. But since this held for \emph{any} way of expanding the interpretation, it must be that `$\forall x\, \atom{H}{x}$' is true in the old interpretation. We've assumed nothing about the interpretation except that it was one in which `$\forall x(\atom{H}{x} \eand \atom{J}{x})$'  is true, so any interpretation in which `$\forall x(\atom{H}{x} \eand \atom{J}{x})$' is true is one in which `$\forall x\, \atom{H}{x}$' is true. The argument is valid!
\end{quote}
