...
\item The FOL sentences $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$ are \define{jointly consistent} iff there is some interpretation in which all of the sentences are true. They are \define{jointly inconsistent} iff there is no such interpretation.
...
Now `$\atom{A}{d,d}$' is true, so it is surely true that `$\exists x\, \atom{A}{x,x}$'. Next, we want `$\atom{B}{d}$' to be false, so the referent of `$d$' must not be in the extension of `$B$'. We might simply offer:
	...
	\end{ekey}
Now we have an interpretation where `$\exists x\, \atom{A}{x,x}$' is true, but where `$\atom{B}{d}$' is false. So there is an interpretation where `$\exists x\, \atom{A}{x,x} \eif \atom{B}{d}$' is false. So `$\exists x\, \atom{A}{x,x} \eif \atom{B}{d}$' is not a logical truth.
...
Does this interpretation make the premise true? Yes it does! Note that `$\atom{G}{a} \eif \atom{G}{a}$' is true. (Indeed, it is a logical truth.) But then certainly `$\exists x (\atom{G}{x} \eif \atom{G}{a})$' is true, so the premise is true, and the conclusion is false, in this interpretation. The argument is therefore invalid. 

In passing, note that we have also shown that `$\exists x(\atom{G}{x} \eif \atom{G}{a})$' does \emph{not} entail `$\exists x\, \atom{G}{x} \eif \atom{G}{a}$'. Equally, we have shown that the sentences `$\exists x (\atom{G}{x} \eif \atom{G}{a})$' and `$\enot (\exists x\, \atom{G}{x} \eif \atom{G}{a})$' are jointly consistent.
...
and many more besides.
	...
	It will also show that $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$, $\enot \meta{C}$ are jointly consistent.}
...
Sometimes, we can reason about all interpretations fairly easily. For example, we can offer a relatively simple argument that `$\atom{R}{a,a}\eiff \atom{R}{a,a}$' is a logical truth:
	\begin{quote}
		\label{allmodels1}
		Any relevant interpretation will give `$\atom{R}{a,a}$' a truth value. If `$\atom{R}{a,a}$' is true in an interpretation, then `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in that interpretation. If `$\atom{R}{a,a}$' is false in an interpretation, then `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in that interpretation. These are the only alternatives. So `$\atom{R}{a,a} \eiff \atom{R}{a,a}$' is true in every interpretation. Therefore, it is a logical truth.
	\end{quote}
...
Here is another bit of reasoning. Consider the sentence `$\forall x(\atom{R}{x,x}\eiff \atom{R}{x,x})$'. Again, it should obviously be a logical truth, but to say precisely why is quite a challenge. We cannot say that `$\atom{R}{x,x} \eiff \atom{R}{x,x}$' is true in every interpretation, since `$\atom{R}{x,x} \eiff \atom{R}{x,x}$' is not even a \emph{sentence} of FOL (remember that `$x$' is a variable, not a name). So we have to be a bit cleverer. 
	\begin{quote}
		Consider some arbitrary interpretation. Consider some arbitrary member of the domain, which, for convenience, we will call \emph{obbie}, and suppose we extend our original interpretation by adding a new name, `$c$', to name \emph{obbie}. Then either `$\atom{R}{c,c}$' will be true or it will be false. If `$\atom{R}{c,c}$' is true, then `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' is true. If `$\atom{R}{c,c}$' is false, then `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' will be true. So either way, `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' is true. Since there was nothing special about \emph{obbie}---we might have chosen any object---we see that no matter how we extend our original interpretation by allowing `$c$' to name some new object, `$\atom{R}{c,c} \eiff \atom{R}{c,c}$' will be true in the new interpretation. So `$\forall x (\atom{R}{x,x} \eiff \atom{R}{x,x})$' was true in the original interpretation. But we chose our interpretation arbitrarily, so `$\forall x (\atom{R}{x,x} \eiff \atom{R}{x,x})$' is true in every interpretation. It is therefore a logical truth.
	\end{quote}
