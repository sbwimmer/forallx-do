...
So, in particular:
	\begin{ebullet}
		\item `$\atom{C}{x}$' is to be true of all and only those things which teach Logic III in Calgary (whatever those things might be).
	\end{ebullet}
...
We can stipulate directly what predicates are to be true of, so it is worth noting that our stipulations can be as arbitrary as we like. For example, we could stipulate that `$\atom{H}{x}$' should be true of, and only of, the following objects:
	\begin{center}
		Justin Trudeau\\
		the number $\pi$\\
		every top-F key on every piano ever made
	\end{center}
Now, the objects that we have listed have nothing particularly in common. But this doesn't matter. Logic doesn't care about what strikes us mere humans as `natural' or `similar'. Armed with this interpretation of `$\atom{H}{x}$', suppose we now add to our symbolization key:
	\begin{ekey}
		\item[j] Justin Trudeau
		\item[r] Rachel Notley
		\item[p] the number $\pi$
	\end{ekey}
Then `$\atom{H}{j}$' and `$\atom{H}{p}$' will both be true, on this interpretation, but `$\atom{H}{r}$' will be false, since Rachel Notley was not among the stipulated objects. 
...
That is an indirect stipulation. What about a direct stipulation? This is slightly harder. If we \emph{simply} list objects that fall under `$\atom{L}{x,y}$', we will not know whether they are the lover or the beloved (or both). We have to find a way to include the order in our explicit stipulation. 
...
Here the angle-brackets keep us informed concerning order. Suppose we now add the following stipulations:
	\begin{ekey}
		...
		\item[r] Sartre
	\end{ekey}
Then `$\atom{B}{l,m}$' will be true, since \ntuple{Lenin, Marx} was in our explicit list, but `$\atom{B}{m,l}$' will be false, since \ntuple{Marx, Lenin} was not in our list. However, both `$\atom{B}{h,r}$' and `$\atom{B}{r,h}$' will be true, since both \ntuple{Heidegger, Sartre} and \ntuple{Sartre, Heidegger} are in our explicit list.
...
Suppose we want to consider just a single two-place predicate, `$\atom{R}{x,y}$'. Then we can represent it just by drawing an arrow between two objects, and stipulate that `$\atom{R}{x,y}$' is to hold of $x$ and $y$ just in case there is an arrow running from $x$ to $y$ in our diagram. As an example, we might offer:
\begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2) at (2,2) {2};
...
\end{tikzpicture}
\end{center}
This would be suitable to characterize an interpretation whose domain is the first four positive whole numbers, and which interprets `$\atom{R}{x,y}$' as being true of and only of:
	\begin{center}
		\ntuple{1, 2}, 
		\ntuple{2, 3}, 
		\ntuple{3, 4}, 
...
\end{tikzpicture}
\end{center}
for an interpretation with the same domain, which interprets `$\atom{R}{x,y}$' as being true of and only of:
	\begin{center}
		\ntuple{1, 3}, 
		\ntuple{3, 1}, 
		\ntuple{3, 4}, 
...
Dealing with atomic sentences, then, is very intuitive. When \meta{R} is an $n$-place predicate and $\meta{a}_1$, $\meta{a}_{2}$, \dots, $\meta{a}_{n}$ are names, 

	\factoidbox{
		$\atom{\meta{R}}{\meta{a}_{1},\meta{a}_{2},\dots,\meta{a}_{n}}$ is true in an interpretation \textbf{iff}\\
		$\meta{R}$ is true of the objects named by $\meta{a}_{1}$, $\meta{a}_{2}$, \dots, $\meta{a}_{n}$ in that interpretation (considered in that order)
	}
Recall, though, that there is a special kind of atomic sentence: two names connected by an identity sign constitute an atomic sentence. This kind of atomic sentence is also easy to handle. Where \meta{a} and \meta{b} are any names, 
	\factoidbox{
...
Unfortunately, this na\"{i}ve thought is not general enough. For example, we want to be able to say that `$\forall x \exists y\, \atom{L}{x,y}$' is true just in case `$\exists y\, \atom{L}{x,y}$' is true of everything in the domain. This is problematic, since our interpretation does not directly specify what `$\exists y\, \atom{L}{x,y}$' is to be true of. Instead, whether or not this is true of something should follow just from the interpretation of `$\atom{L}{x,y}$', the domain, and the meanings of the quantifiers. 

So here is a second na\"{i}ve thought. We might try to say that `$\forall x \exists y\, \atom{L}{x,y}$' is to be true in an interpretation iff $\exists y\, \atom{L}{\meta{a},y}$ is true for \emph{every} name \meta{a} that we have included in our interpretation. Similarly, we might try to say that $\exists y\, \atom{L}{\meta{a},y}$ is true just in case $\atom{L}{\meta{a},\meta{b}}$ is true for \emph{some} name \meta{b} that we have included in our interpretation.
...
Suppose that \meta{A} is a formula containing at least one occurrence of the variable \meta{x}, and that $\meta{x}$ is free in $\meta{A}$. We will write this thus:
...
for the formula obtained by replacing \emph{every} occurrence of $\meta{x}$ in \meta{A} with $\meta{c}$. The resulting formula is called a \define{substitution instance} of $\forall \meta{x}\meta{A}$ and $\exists\meta{x}\meta{A}$.  Also, $\meta{c}$ is called the \define{instantiating name}. So:
...
Our interpretation will include a specification of which names correspond to which objects in the domain. Take any object in the domain, say, $d$, and a name $\meta{c}$ which is not already assigned by the interpretation. If our interpretation is $\mathbf{I}$, then we can consider the interpretation $\mathbf{I}[d/\meta{c}]$ which is just like $\mathbf{I}$ except it \emph{also} assigns the name $\meta{c}$ to the object~$d$. Then we can say that $d$ \define{satisfies} the formula $\meta{A}(\dots\meta{x}\dots\meta{x}\dots)$ in the interpretation~$\mathbf{I}$ if, and only if, $\meta{A}(\dots\meta{c}\dots\meta{c}\dots)$ is true in $\mathbf{I}[d/\meta{c}]$. (If $d$ satisfies $\meta{A}(\dots\meta{x}\dots\meta{x}\dots)$ we also say that $\meta{A}(\dots\meta{x}\dots\meta{x}\dots)$ is \emph{true of}~$d$.) So, for instance, Socrates satisfies the formula~$\atom{P}{x}$ since $\atom{P}{c}$ is true in the interpretation $\mathbf{I}[\mathrm{Socrates}/c]$, i.e., the interpretation:
\begin{ekey}
	\item[\text{domain}] all people born before 2000\textsc{ce}
	\item[a] Aristotle
	\item[b] Beyonc\'e
...
Armed with this notation, the rough idea is as follows. The sentence $\forall \meta{x}\meta{A}(\ldots \meta{x} \ldots \meta{x} \ldots)$ will be true in $\mathbf{I}$ iff $\meta{A}(\ldots \meta{c} \ldots \meta{c}\ldots)$ is true in $\mathbf{I}[d/\meta{c}]$, i.e., no matter what object (in the domain) we name with $\meta{c}$. In other words, $\forall \meta{x} \meta{A}(\ldots \meta{x} \ldots \meta{x} \ldots)$ is true iff every object in the domain satisfies $\meta{A}(\ldots \meta{x} \ldots \meta{x} \ldots)$. Similarly, the sentence $\exists \meta{x}\meta{A}$ will be true iff there is \emph{some} object that satisifes $\meta{A}(\ldots \meta{x} \ldots \meta{x} \ldots)$, i.e., $\meta{A}(\ldots \meta{c} \ldots \meta{c} \ldots)$ true in $\mathbf{I}[d/c]$ for some object~$d$.
	\factoidbox{
		$\forall \meta{x}\meta{A}(\ldots \meta{x}\ldots\meta{x}\ldots)$ is true in an interpretation \textbf{iff}\\ 
		every object in the domain satisfies $\meta{A}(\ldots \meta{x} \ldots \meta{x}\ldots)$.
		
		\
		\\
		$\exists \meta{x}\meta{A}(\ldots \meta{x}\ldots\meta{x}\ldots)$ is true in an interpretation \textbf{iff}\\ at least one object in the domain satisfies
		$\meta{A}(\ldots \meta{x}\ldots\meta{x}\ldots)$.
	}
...
Finally, let us note that the concept of an object satisfying a formula with a free variable can also be extended to formulas with more than one free variable. If we have a formula $\meta{A}(\meta{x},\meta{y})$ with two free variables $\meta{x}$ and $\meta{y}$, then we can say that a pair of objects $\langle a, b\rangle$ satisfies $\meta{A}(\meta{x},\meta{y})$ iff $\meta{A}(\meta{c},\meta{d})$ is true in the interpretation extended by two names $\meta{c}$ and $\meta{d}$, where $\meta{c}$ names~$a$ and $\meta{d}$ names~$b$. So, for instance, $\langle \mathrm{Socrates}, \mathrm{Plato}\rangle$ satisfies $R(x,y)$ since $R(c,d)$ is true in the interpretation:
\begin{ekey}
	\item[\text{domain}] all people born before 2000\textsc{ce}
	\item[a] Aristotle
	\item[b] Beyonc\'e
...
means that there is no interpretation in which all of $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$ are true and in which \meta{C} is false. Derivatively,
	$$\entails\meta{A}$$
...
\begin{itemize}
\item An FOL sentence $\meta{A}$ is a \define{validity} iff $\meta{A}$ is true in every interpretation; i.e.,  $\entails\meta{A}$.
...
\item Two FOL sentences \meta{A} and \meta{B} are \define{equivalent} iff they are true in exactly the same interpretations as each other; i.e., both $\meta{A}\entails\meta{B}$ and $\meta{B}\entails\meta{A}$.
...
\item The FOL sentences $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$ are \define{jointly satisfiable} iff there is some interpretation in which all of the sentences are true. They are \define{jointly unsatisfiable} iff there is no such interpretation.
...
Suppose we want to show that `$\forall x\, \atom{S}{x}$' and `$\exists x\, \atom{S}{x}$' are not logically equivalent. We need to construct an interpretation in which the two sentences have different truth values; we want one of them to be true and the other to be false. We start by specifying a domain. Again, we make the domain small so that we can specify extensions easily. In this case, we will need at least two objects. (If we chose a domain with only one member, the two sentences would end up with the same truth value. In order to see why, try constructing some partial interpretations with one-member domains.) For concreteness, let's take:
	...
	\end{ekey}
We can make `$\exists x\, \atom{S}{x}$' true by including something in the extension of `$S$', and we can make `$\forall x\, \atom{S}{x}$' false by leaving something out of the extension of `$S$'. For concreteness we will offer:
	\begin{ekey}
...
Now `$\exists x\, \atom{S}{x}$' is true, because `$\atom{S}{x}$' is true of Ornette Coleman. Slightly more precisely, extend our interpretation by allowing `$c$' to name Ornette Coleman.  `$\atom{S}{c}$' is true in this extended interpretation, so `$\exists x\, \atom{S}{x}$' was true in the original interpretation. Similarly, `$\forall x\, \atom{S}{x}$' is false, because `$\atom{S}{x}$' is false of Miles Davis. Slightly more precisely, extend our interpretation by allowing `$d$' to name Miles Davis, and `$\atom{S}{d}$' is false in this extended interpretation, so `$\forall x\, \atom{S}{x}$' was false in the original interpretation. We have provided a counter-interpretation to the claim that `$\forall x\, \atom{S}{x}$' and `$\exists x\, \atom{S}{x}$' are logically equivalent.
	\factoidbox{
		To show that $\meta{A}$ is not a validity, it suffices to find an interpretation where $\meta{A}$ is false.
...
In passing, note that we have also shown that `$\exists x(\atom{G}{x} \eif \atom{G}{a})$' does \emph{not} entail `$\exists x\, \atom{G}{x} \eif \atom{G}{a}$'. Equally, we have shown that the sentences `$\exists x (\atom{G}{x} \eif \atom{G}{a})$' and `$\enot (\exists x\, \atom{G}{x} \eif \atom{G}{a})$' are jointly satisfiable.
...
Again, we want to show that this is invalid. To do this, we must make the premises true and the conclusion false. Here is a suggestion:
	\begin{ekey}
		...
		\item[\atom{L}{x,y}] \gap{x} is in a civil partnership with \gap{y}
	\end{ekey}
The premise is clearly true on this interpretation. Anyone in the domain is a UK citizen in a civil partnership with some other UK citizen. That other citizen will also, then, be in the domain. So for everyone in the domain, there will be someone (else) in the domain with whom they are in a civil partnership. Hence `$\forall x \exists y\, \atom{L}{x,y}$' is true. However, the conclusion is clearly false, for that would require that there is some single person who is in a civil partnership with everyone in the domain, and there is no such person, so the argument is invalid. We observe immediately that the sentences `$\forall x \exists y\, \atom{L}{x,y}$' and `$\enot\exists y \forall x\, \atom{L}{x,y}$' are jointly satisfiable and that `$\forall x \exists y\, \atom{L}{x,y}$' does not entail `$\exists y \forall x\, \atom{L}{x,y}$'. 
...
and many more besides.
	...
	\factoidbox{
	To show that $\meta{A}_1, \meta{A}_2, \ldots, \meta{A}_n \therefore \meta{C}$ is invalid, it suffices to find an interpretation where all of $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$ are true and where $\meta{C}$ is false.
	
	That same interpretation will show that $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$ do not entail~$\meta{C}$.
	
	It will also show that $\meta{A}_1$, $\meta{A}_2$, \dots, $\meta{A}_n$, $\enot \meta{C}$ are jointly satisfiable.}
When you provide an interpretation to refute a claim---to show that a sentence is not a validity, say, or that an entailment fails---this is sometimes called providing a \emph{counter-interpretation} (or providing a \emph{counter-model}).
...
\section{Validities and contradictions}
We can show that a sentence is \emph{not} a validity just by providing one carefully specified interpretation: an interpretation in which the sentence is false. To show that something is a validity, on the other hand, it would not be enough to construct ten, one hundred, or even a thousand interpretations in which the sentence is true. A sentence is only a validity if it is true in \emph{every} interpretation, and there are infinitely many interpretations. We need to reason about all of them, and we cannot do this by dealing with them one by one!
...
Here is another bit of reasoning. Consider the sentence `$\forall x(\atom{R}{x,x}\eor\enot \atom{R}{x,x})$'. Again, it should obviously be a validity, but to say precisely why is quite a challenge. We cannot say that `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$' is true in every interpretation, since `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$' is not even a \emph{sentence} of FOL (remember that `$x$' is a variable, not a name). So we have to be a bit cleverer. 
	\begin{quote}
		Consider some arbitrary interpretation. $\forall x(\atom{R}{x,x}\eor \enot\atom{R}{x,x})$ is true in our interpretation iff $\atom{R}{x,x}\eor\enot\atom{R}{x,x}$ is satisfied by every object of its domain. Consider some arbitrary member of the domain, which, for convenience, we will call Fred. Either Fred satisfies $\atom{R}{x,x}$ or it does not. If Fred satisfies `$\atom{R}{x,x}$', then Fred also satisfies `$\atom{R}{x,x} \eor \enot \atom{R}{c,c}$'. If Fred does not satisfy `$\atom{R}{x,x}$', it \emph{does} satisfy `$\enot\atom{R}{x,x}$' and so also `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$'.\footnote{We use here the fact that the truth conditions for connectives also apply to satisfaction: $a$ satisfies $\meta{A}(\meta{x}) \lor \meta{B}(\meta{x})$ iff $a$ satisfies $\meta{A}(\meta{x})$ or $\meta{B}(\meta{x})$, etc.} So either way, Fred satisfies `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$'. Since there was nothing special about Fred---we might have chosen any object---we see that every object in the domain satisfies `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$'. So `$\forall x (\atom{R}{x,x} \eor\enot \atom{R}{x,x})$' is true in our interpretation. But we chose our interpretation arbitrarily, so `$\forall x (\atom{R}{x,x} \eor\enot \atom{R}{x,x})$' is true in every interpretation. It is therefore a validity.
	\end{quote}
...
After all, if everything is both $H$ and $J$, then everything is~$H$. But we can only show that the argument is valid by considering what must be true in every interpretation in which the premise is true. To show this, we would have to reason as follows:
	\begin{quote}
		Consider an arbitrary interpretation in which the premise `$\forall x(\atom{H}{x} \eand \atom{J}{x})$' is true. It follows that `$\atom{H}{x} \eand \atom{J}{x}$' is satisfied by every object in this interpretation. `$\atom{H}{x}$' will, then, also be satisfied by every object.\footnote{Here again we make use of the fact that any object that satisfies $\meta{A}(\meta{x}) \land \meta{B}(\meta{x})$ must satisfy both $\meta{A}(\meta{x})$ and $\meta{B}(\meta{x})$.} So it must be that `$\forall x\, \atom{H}{x}$' is true in the  interpretation. We've assumed nothing about the interpretation except that it was one in which `$\forall x(\atom{H}{x} \eand \atom{J}{x})$'  is true, so any interpretation in which `$\forall x(\atom{H}{x} \eand \atom{J}{x})$' is true is one in which `$\forall x\, \atom{H}{x}$' is true. The argument is valid!
\end{quote}
