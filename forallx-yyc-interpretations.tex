...
Dealing with atomic sentences, then, is very intuitive. When \metav{R} is an $n$-place predicate and $\metav{a}_1$, $\metav{a}_{2}$, \dots, $\metav{a}_{n}$ are names, 
	...
	\factoidbox{
		$\atom{\metav{R}}{\metav{a}_{1},\metav{a}_{2},\dots,\metav{a}_{n}}$ is true in an interpretation \textbf{iff}\\
		$\metav{R}$ is true of the objects named by $\metav{a}_{1}$, $\metav{a}_{2}$, \dots, $\metav{a}_{n}$ in that interpretation (considered in that order)
	}
...
for the formula obtained by replacing \emph{every} occurrence of $\metav{x}$ in \metav{A} with $\metav{c}$. The resulting formula is called a \define{substitution instance} of $\forall \metav{x}\metav{A}$ and $\exists\metav{x}\metav{A}$.  Also, $\metav{c}$ is called the \define{instantiating name}. So:
...
\item The FOL sentences $\metav{A}_1$, $\metav{A}_2$, \dots, $\metav{A}_n$ are \define{jointly satisfiable} iff there is some interpretation in which all of the sentences are true. They are \define{jointly unsatisfiable} iff there is no such interpretation.
...
and many more besides.
	...
	\factoidbox{
	To show that $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n \therefore \metav{C}$ is invalid, it suffices to find an interpretation where all of $\metav{A}_1$, $\metav{A}_2$, \dots, $\metav{A}_n$ are true and where $\metav{C}$ is false.
	
	That same interpretation will show that $\metav{A}_1$, $\metav{A}_2$, \dots, $\metav{A}_n$ do not entail~$\metav{C}$.
	
	It will also show that $\metav{A}_1$, $\metav{A}_2$, \dots, $\metav{A}_n$, $\enot \metav{C}$ are jointly satisfiable.}
