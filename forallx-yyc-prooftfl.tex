...
\section{Working backwards from what we want}
...
For example: If your conclusion is a conditional $\meta{A}\eif\meta{B}$, plan to use the {\eif}I rule. This requires starting a subproof in which you assume \meta{A}. The subproof ought to end with \meta{B}. Then, continue by thinking about what can you do to get $\meta{B}$ inside that subproof, and how you can use the assumption $\meta{A}$.
...
If we want to prove $\meta{A} \eand \meta{B}$, working backward means we should write $\meta{A} \eand \meta{B}$ at the bottom of our proof, and try to prove it using $\eand$I. At the top, we'll write out the premises of the proof, if there are any. Then, at the bottom, we write the sentence we want to prove. If it is a conjunction, we'll prove it using $\eand$I.
  \begin{proof}
	\have{1}{\meta{P}_1}
	\ellipsesline 
	\hypo[k]{k}{\meta{P}_k}
\ellipsesline
  ...
  \end{proof}
For $\eand$I, we need to prove $\meta{A}$ first, then prove $\meta{B}$. For the last line, we'll cite the lines where we proved $\meta{A}$ and  $\meta{B}$, and use $\eand$I.The parts of the proof labelled $\vdots$ have to still be filled in. We'll mark the line numbers $m$, $n$ for now. When the proof is complete, these placeholders can be replaced by actual numbers.

If our goal is to prove a conditional $\meta{A} \eif \meta{B}$, we'll have to use $\eif$I. This requires a subproof starting with $\meta{A}$ and ending with~$\meta{B}$. We'll set up our proof as follows:
\begin{proof}
\open
\hypo[n]{2}{\meta{A}}
\ellipsesline 
...
If we want to prove $\enot \meta{A}$, we'll have to use $\enot$I.
\begin{proof}
\open
\hypo[n]{2}{\meta{A}}
\ellipsesline 
...
\have{4}{\enot \meta{A}}\ni{2-3}
\end{proof} 
For $\enot$I, we have to start a subproof with assumption $\meta{A}$; the last line of the subproof has to be $\ered$.  We'll cite the subproof, and use $\enot$I.

When working backwards, continue to do so until as long as you can. So if you're working backwards to prove $\meta{A} \eif \meta{B}$ and have set up a subproof in which you want to prove $\meta{B}$, now look at it~$\meta{B}$. If, say, it is a conjunction, work backwards from it, and write down the two conjuncts inside your subproof.

Of course, you can also work backward from a disjunction $\meta{A} \lor \meta{B}$, if that is your goal.
The $\eor$I rule requires that you have one of the disjuncts in order to infer $\meta{A} \eor \meta{B}$.
So to work backwards, you pick a disjunct, infer $\meta{A} \eor \meta{B}$ from it, and then continue to look for a proof of the disjunct you picked:
\begin{proof}
	\ellipsesline
	\have[n]{2}{\meta{A}} 
	\have{3}{\meta{A} \lor \meta{B}}\oi{2}
\end{proof}
...
\begin{proof}
	...
	\have{3}{\meta{A} \lor \meta{B}}\oi{2}
\end{proof}
Obviously, deleting everything and starting over is frustrating, so you should avoid it. If your goal is a disjunction, therefore, you should \emph{not} start by working backwards: try working forwards first, and apply the $\eor$I strategy only when working forwards (and working backwards using $\eand$I, $\eif$I, and $\enot$I) no longer work.

\section{Work forwards from what you have}
...
If we want to use a sentence of the form $\meta{A} \eand \meta{B}$ we use $\eand$E. That rule allows us to do two things: infer $\meta{A}$, and infer $\meta{B}$. So in a proof where we have $\meta{A} \eand \meta{B}$, we can always work forwards by writing $\meta{A}$ and $\meta{B}$ immediately below the conjunction:
\begin{proof}
	\ellipsesline
...
Working forwards from a disjunction works a bit differently. To use a disjunction, we use the $\eor$E rule. But in order to apply that rule, it is not enough to know what the disjuncts of the disjunction are that we want to use. We must also keep in mind what it is that we want to prove. Suppose we want to prove $\meta{C}$, and we have $\meta{A} \lor B$ to work with. (Again, that $\meta{A} \lor B$ may be a premise of the proof, an assumption of a subproof, or something already proved, e.g., by $\eand$E.) In order to be able to apply the $\eor$E rule, we'll have to set up two subproofs.
\begin{proof}
	\ellipsesline
	...
	\ellipsesline
\end{proof} 
The first subproof starts with the first disjunct, $\meta{A}$, and ends with the sentence we're looking for, $\meta{C}$. The second subproof starts with the other disjunct, $\meta{B}$, and also ends with the goal sentence~$\meta{C}$. Each of these subproofs have to be filled in further. We
can justify $\meta{C}$ by using $\eor$E, citing the line with $\meta{A}
\eor \meta{B}$ and the two subproofs.

In order to use a conditional $\meta{A} \eif \meta{B}$, you need the antecedent $\meta{A}$ in order to apply $\eif$E. So to work forward from a conditional, you will derive $\meta{B}$, justify it by $\eif$E, and set up $\meta{A}$ as a new subgoal.
\begin{proof}
	\ellipsesline
...
Finally, to use a negated sentence $\enot \meta{A}$, you would apply $\enot$E. It requires, in addition to $\enot \meta{A}$ also the corresponding sentence $\meta{A}$ without the negation. The sentence you'll get is always the same: $\ered$. So working forward from a negated sentence works especially well inside a subproof that you'll want to use for $\enot$I (or IP).
\begin{proof}
	\ellipsesline
...
Suppose we want to show that the argument $(A \eand B) \lor (A \eand C) \therefore A \eand (B \lor C)$ is valid. We start the proof by writing the premise and conclusion down. (On a piece of paper, you would want as much space as possible between them, so write the premises at the top of the sheet and the conclusion at the bottom.)
\begin{proof}
   \hypo{1}{(A \eand B) \lor (A \eand C)}
\ellipsesline
  \have[n]{2}{A \eand (B \lor C)}
\end{proof}
...
\begin{proof}
	...
	\hypo{2}{A \eand B}
	\ellipsesline 
	\have[n]{6}{A \eand (B \lor C)}
	\close
	\open
	\hypo{7}{A \eand C}
	\ellipsesline
	\have[m]{11}{A \eand (B \lor C)}
	\close
	\have{12}{A \eand (B \lor C)}\oe{1,2-6,7-11}
\end{proof}
In the first subproof, we now work backwards from the conclusion and set up the sub goals for proving line 6 using $\eand$I.
\begin{proof}
	\hypo{1}{(A \eand B) \lor (A \eand C)}
	\open
	\hypo{2}{A \eand B}
	...
	\ellipsesline
	\have[n][-1]{5}{B \lor C}
	\have[n]{6}{A \eand (B \lor C)}\ai{4,5}
	\close
	\open
	\hypo{7}{A \eand C}
	\ellipsesline
	...
	\have{12}{A \eand (B \lor C)}\oe{1,2-6,(7)-11}
\end{proof}
We immediately see that we get line $i$ from 2 by $\eand$E; let's apply the strategy for proving disjunctions to line $n-1$: look for a proof of $B$. (We have a choice of which disjunct to pick, but picking $C$ wouldn't work and we'd end up having to backtrack.)
\begin{proof}
	\hypo{1}{(A \eand B) \lor (A \eand C)}
	\open
	\hypo{2}{A \eand B}
	...
	\have{12}{A \eand (B \lor C)}\oe{1,2-6,7-11}
\end{proof}
Like line $i$ (i.e., 3), we get line 4 from 2 by $\eand$E. That's it for
the first subproof. The second subproof is almost exactly the same. We'll leave it as an exercise.

Remember that when we started, we had the option of working forward from the premise, or working backward from the conclusion, and we picked the first option. The second option also leads to a proof, but it will look different.  The first steps would be to split the conclusion apart and set up two subgoals, $A$ and $B \lor C$, and then work forwards from the premise to prove them, e.g.,:
\begin{proof}
	\hypo{1}{(A \eand B) \lor (A \eand C)}
	\open
	\hypo{2}{A \eand B}
	...
	\have[m][-1]{10}{B \lor C}
	\close
	\have{11}{B \lor C}\oe{1,(7)-8,(9)-(10)}	
	\have{12}{A \eand (B \lor C)}\ai{6,10}
\end{proof}

Let's give another example to illustrate how to apply the strategies to deal with conditionals and negation. The sentence $(A \eif B) \eif (\enot B \eif \enot A)$ is a tautology; let's see if we can find a proof of it, from no premises, using the strategies. We first write the sentence at the bootom of a sheet of paper. Since working forwards is not an option, we work backwards, and set up a subproof to establish the sentence we want using $\eif$I. Its assumption must be the antecedent of the conditional we want to prove, and its last line the consequent.
\begin{proof}
\open
\hypo{1}{A \eif B}
\ellipsesline
...
\end{proof}
The new goal, $\enot B \eif \enot A$ is itself a conditional, so working backwards we set up another subproof:
\begin{proof}
	\open
	\hypo{1}{A \eif B}
	\open
	...
	\have{8}{(A \eif B) \eif (\enot B \eif \enot A)}\ci{1-7}
\end{proof}
From $\enot A$ we again work backwards:
\begin{proof}
	\open
	\hypo{1}{A \eif B}
	\open
	...
	\have{8}{(A \eif B) \eif (\enot B \eif \enot A)}\ci{1-7}
\end{proof}
To prove $\ered$ we now work forwards from the one negated sentence we have, $\enot B$ on line 2. That means we have to derive $B$---which we get in turn by working forwards from $A \eif B$, since $B$ is the consequent of that conditional. Its antecedent $A$ is already available on line~3. So we finish with:
\begin{proof}
	\open
	\hypo{1}{A \eif B}
	\open
...
In very many cases, the strategies of working forwards and backwards will eventually pan out. But there are cases where they do not work.  If you cannot find a way to show $\meta{A}$ directly using those, use IP instead. To do this, set up a subproof in which you assume $\enot\meta{A}$ and look for a proof of $\ered$ inside that subproof.
...
\end{proof}
Here, we have to start a subproof with assumption $\enot \meta{A}$;
the last line of the subproof has to be $\ered$.
We'll cite the subproof, and use IP.  In the subproof, we now
have an additional assumption (on line $n$) to work with.

Suppose we used the indirect proof strategy, or we're in some other situation
where we're looking for a proof of $\ered$.  What's a good candidate?
Of course the obvious candidate would be to use a negated sentence, since (as we saw above)
$\enot$E always yields~$\ered$. If you are using the IP strategy, this results in the following paradoxical situation: 
\begin{proof}
\ellipsesline
\open
...
\end{proof} 
This looks weird: We wanted to prove $A$ and the strategies failed us; so we used IP as a last resort. And now we find ourselves in the same situation: looking for a proof of~$\meta{A}$. But remember that we are now inside a subproof, and in that subproof we have an additional assumption ($\enot \meta{A}$) to work with which we didn't have before.

\section{Indirect proof of excluded middle}\label{s:proofLEM}

The sentence $A \eor \enot A$ is a tautology, and so should have a proof even without any premises. But working backwards does not work: to get $A \eor \enot A$ using $\eor$I we would have to prove either $A$ or $\enot A$---again, from no premises. But neither of these is a tautology, so we won't be able to prove either. Working forwards doesn't work either, since there is nothing to work forwards from. So, the only option is indirect proof.
\begin{proof}
	\open
	...
	\have[m]{8}{\ered}
	\close
	\have{9}{A \lor \enot A}\ip{2-8}
\end{proof}
Now we do have something to work forward from: the assumption $\enot(A \eor \enot A)$. To use it, we justify $\ered$ by $\enot$E, citing the assumption, in addition to the corresponding unnegated sentence (yet to be proved).
\begin{proof}
	\open
	\hypo{2}{\enot (A \eor \enot A)}
	\ellipsesline
	\have[m][-1]{7}{A \lor \enot A}
	\have{8}{\ered}\ne{2,7}
	\close
	\have{9}{A \lor \enot A}\ip{2-8}
\end{proof}
At the outset, working backwards to prove $A \eor\enot A$ by $\eor$I did not work. But we are now in a different situation: we want to prove $A \eor\enot A$ inside a subproof. In general, when dealing with new goals we should go back and start with the basic strategies. In this case, we should pick a disjunct and try to prove it. Let's pick $\enot A$. Then working backward from that, we start another subproof in order to justify $\enot A$ using $\enot$I.
\begin{proof}
	\open
	\hypo{2}{\enot (A \eor \enot A)}
	\open
	...
	\close
	\have{6}{\enot A}\ni{3-(5)}
	\have{7}{A \lor \enot A}\oi{6}
	\have{8}{\ered}\ne{2,7}
	\close
	\have{9}{A \lor \enot A}\ni{2-8}
\end{proof}
Inside this new subproof, we need to justify $\ered$. Again, the best way to do this is to use a negated sentence, and $\enot(A \eor \enot A)$ is the only negated sentence we can use. The corresponding unnegated sentence, $A \eor \enot A$, however, directly follows from $A$ by $\eor$I. Our complete proof is:
\begin{proof}
	\open
	\hypo{2}{\enot (A \eor \enot A)}
	\open
...
\end{proof}
(We gave a proof of $\meta{A} \eor \enot\meta{A}$ using only our basic rules in \S\ref{s:proofLEM}.)
...
\begin{proof}
  ...
  \have[l+3]{bbbb}{\ered}\ne{b, bbb}
  \close
  \have[l+4]{na}{\enot\meta{A}}\ni{a-aaaa}
  \have[l+5]{nna}{\enot\enot\meta{A}}\ni{b-bbbb}
  \have[l+6]{bot}{\ered}\ne{nna, na}
  \close
...
\problempart
Give a proof of $\meta{A} \lor \enot\meta{A}$. Then
...
Being a theorem 		& Derive the sentence  						& Find the false line in the truth table for the sentence \\ \hline
