...
\end{proof}}
This rule is also sometimes called \emph{modus ponens}. Again, this is an elimination rule, because it allows us to obtain a sentence that may not contain `$\eif$', having started with a sentence that did contain `$\eif$'. Note that the conditional $\meta{A}\eif\meta{B}$ and the antecedent~$\meta{A}$ can be separated from one another in the proof, and they can appear in any order. However, in the citation for $\eif$E, we always cite the conditional first, followed by the antecedent.
...
The general pattern at work here is the following. We first make an additional assumption, $\meta{A}$; and from that additional assumption, we prove~$\meta{B}$. In that case, we know the following: If~$\meta{A}$ is true, then~$\meta{B}$ is true. This is wrapped up in the rule for conditional introduction:
\factoidbox{
	\begin{proof}
		\open
			\hypo[i]{a}{\meta{A}} 
...
So you're trying to find a proof of some conclusion~$\meta{C}$, which will be the last line of your proof. The first thing you do is look at~$\meta{C}$ and ask what the introduction rule is for its main logical operator. This gives you an idea of what should happen \emph{before} the last line of the proof. The justifications for the introduction rule require one or two other sentences above the last line, or one or two subproofs. Moreover, you can tell from~$\meta{C}$ what those sentences are, or what the assumptions and conclusions of the subproof(s) are. Then you can write down those sentence or outline the subproof(s) above the last line, and treat those as your new goals.

For example: If your conclusion is a conditional $\meta{A}\eif\meta{B}$, plan to use the {\eif}I rule. This requires starting a subproof in which you assume~\meta{A}. The subproof ought to end with~\meta{B}. Then, continue by thinking about what you should do to get $\meta{B}$ inside that subproof, and how you can use the assumption~$\meta{A}$.
...
\ellipsesline
  ...
  \end{proof}
For $\eand$I, we need to prove $\meta{A}$ first, then prove $\meta{B}$. For the last line, we have to cite the lines where we (will have) proved $\meta{A}$ and  $\meta{B}$, and use~$\eand$I. The parts of the proof labelled $\vdots$ have to still be filled in. We'll mark the line numbers $m$, $n$ for now. When the proof is complete, these placeholders can be replaced by actual numbers.
...
\have{4}{\enot \meta{A}}\ni{2-3}
\end{proof} 
For $\enot$I, we have to start a subproof with assumption $\meta{A}$; the last line of the subproof has to be $\ered$.  We'll cite the subproof, and use~$\enot$I as the rule.

When working backward, continue to do so as long as you can. So if you're working backward to prove $\meta{A} \eif \meta{B}$ and have set up a subproof in which you want to prove $\meta{B}$. Now look at~$\meta{B}$. If, say, it is a conjunction, work backward from it, and write down the two conjuncts inside your subproof. Etc.

\subsection*{Working backward from a disjunction}

Of course, you can also work backward from a disjunction $\meta{A} \eor \meta{B}$, if that is your goal.
The $\eor$I rule requires that you have one of the disjuncts in order to infer $\meta{A} \eor \meta{B}$.
So to work backward, you pick a disjunct, infer $\meta{A} \eor \meta{B}$ from it, and then continue to look for a proof of the disjunct you picked:
\begin{proof}
	\ellipsesline
	\have[n]{2}{\meta{A}} 
...
To work forward from a sentence of the form $\meta{A} \eand \meta{B}$, we use $\eand$E. That rule allows us to do two things: infer $\meta{A}$, and infer $\meta{B}$. So in a proof where we have $\meta{A} \eand \meta{B}$, we can work forward by writing $\meta{A}$ and/or $\meta{B}$ immediately below the conjunction:
\begin{proof}
  \have[n]{1}{\meta{A} \eand \meta{B}}
  \have{2}{\meta{A}}\ae{1}
  \have{3}{\meta{B}}\ae{1}
\end{proof}
Usually it will be clear in the particular situation you're in which one of \meta{A} or \meta{B} you'll need. It doesn't hurt, however, to write them both down.

\subsection*{Working forward from a disjunction}

Working forward from a disjunction works a bit differently. To use a disjunction, we use the $\eor$E rule. In order to apply that rule, it is not enough to know what the disjuncts of the disjunction are that we want to use. We must also keep in mind what we want to prove. Suppose we want to prove~$\meta{C}$, and we have $\meta{A} \eor B$ to work with. (That $\meta{A} \eor B$ may be a premise of the proof, an assumption of a subproof, or something already proved.) In order to be able to apply the $\eor$E rule, we'll have to set up two subproofs:
\begin{proof}
...
\end{proof} 
The first subproof starts with the first disjunct, $\meta{A}$, and ends with the sentence we're looking for, $\meta{C}$. The second subproof starts with the other disjunct, $\meta{B}$, and also ends with the goal sentence~$\meta{C}$. Each of these subproofs have to be filled in further. We can then justify the goal sentence $\meta{C}$ by using $\eor$E, citing the line with $\meta{A} \eor \meta{B}$ and the two subproofs.

\subsection*{Working forward from a conditional}

In order to use a conditional $\meta{A} \eif \meta{B}$, you also need the antecedent $\meta{A}$ in order to apply~$\eif$E. So to work forward from a conditional, you will derive $\meta{B}$, justify it by $\eif$E, and set up $\meta{A}$ as a new subgoal.
\begin{proof}
...
In very many cases, the strategies of working forward and backward will eventually pan out. But there are cases where they do not work.  If you cannot find a way to show $\meta{A}$ directly using those, use IP instead. To do this, set up a subproof in which you assume $\enot\meta{A}$ and look for a proof of $\ered$ inside that subproof.
...
Suppose we used the indirect proof strategy, or we're in some other situation where we're looking for a proof of $\ered$.  What's a good candidate? Of course the obvious candidate would be to use a negated sentence, since (as we saw above) $\enot$E always yields~$\ered$. If you set up a proof as above, trying to prove \meta{A} using~IP, you will have $\enot \meta{A}$ as the assumption of your subproof---so working forward from it to justify $\ered$ inside your subproof, you would next set up \meta{A} as a goal inside your subproof. If you are using this IP strategy, you will find yourself in the following situation: 
\begin{proof}
\open
...
\end{proof} 
This looks weird: We wanted to prove $\meta{A}$ and the strategies failed us; so we used IP as a last resort. And now we find ourselves in the same situation: we are again looking for a proof of~$\meta{A}$. But notice that we are now \emph{inside} a subproof, and in that subproof we have an additional assumption ($\enot \meta{A}$) to work with which we didn't have before. Let's look at an example.
...
\problempart
Formulate strategies for working backward and forward from $\meta{A} \eiff \meta{B}$.
...
Indeed, it is a recipe which shows us that, anything we can prove using the rule R, we can prove (with one more line) using just the basic rules of \S\ref{s:BasicTFL} without~R. That is what it means to say that the rule~R can be derived from the other basic rules: anything that can be justified using~R can be justified using only the other basic rules.
...
\problempart
Give a proof of $\meta{A} \eor \enot\meta{A}$. Then
...
\begin{proof}
  ...
  \close
\end{proof}
How could you use it to prove \meta{A} without using IP but with using LEM as well as all the other basic rules?
