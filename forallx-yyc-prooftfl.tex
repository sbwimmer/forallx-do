...
means that there is some proof which starts with assumptions among $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ and ends with $\metav{C}$ (and contains no undischarged assumptions other than those we started with). Derivatively, we will write:
$$\proves \metav{A}$$
to mean that there is a proof of $\metav{A}$ with no assumptions. 
