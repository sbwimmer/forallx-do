...
But we should not simply write:
	\begin{align*}
		A \eor B \eor C
	\end{align*}
In fact, it is a specifc fact about the characteristic truth table of $\eor$ and $\eand$ that guarantees that any two conjunctions (or disjunctions) of the same sentences are tautologically equivalent, however you place the brackets. \emph{But be careful}. These two sentences have \emph{different} truth tables:
	\begin{align*}
		((A \eif B) \eif C)\\
		(A \eif (B \eif C))
	\end{align*}
...
{
name=tautology,
description={A sentnece that has only Ts in the column under the main logical operator of its complete truth table; a sentence that is true on every valuation..}
}
...
\newglossaryentry{tautological equivalence}
{
  name=tautological equivalence,
  text = tautologically equivalent,
...
In \S\ref{s:BasicNotions}, I said that sentences are jointly consistent iff it is possible for all of them to be true at once. We can offer a surrogate for this notion too:
	\factoidbox{
        ...
        \newglossaryentry{tautological consistency}
{
  name=tautological consistency,
  text=jointly tautologically consistent,
description={A property held by sentences if and only if the complete truth table for that set contains one line on which all the sentences are true, i.e., if some valuation makes all the sentences true.}
}
...
The following idea is closely related to that of joint consistency:
	\factoidbox{
       ...
       \newglossaryentry{tautologically valid}
{
  name=tautological validity,
  text = tautologically valid,
...
\noindent\problempart
\label{pr.TT.equiv}
Determine whether each the following statements are tautologically equivalent using complete truth tables. If the two sentences really are tautologically equivalent, write ``equivalent.'' Otherwise write, ``Not equivalent.'' 
\begin{earg}
\item $A$ and $\enot A$
...
\problempart
\label{pr.TT.equiv}
Determine whether each the following sentences are tautologically equivalent using complete truth tables. If the two sentences really are tautologically equivalent, write ``equivalent.'' Otherwise write, ``not equivalent.''
\begin{earg}
...
\begin{earg}
\item $A\eif A \entails  A$ \vspace{.5ex}%invalid

\item $A\eif B$, $B \entails  A$ %invalid

\item $A\eiff B$, $B\eiff C \entails A\eiff C$ %valid

\item $A \eif B$, $A \eif C\entails B \eif C$ %invalid. 

\item $A \eif B$, $B \eif A\entails A \eiff B$ %valid. 
...
Determine whether each argument is valid or invalid, using a complete truth table. 
\begin{earg}
\item $A\eor\bigl[A\eif(A\eiff A)\bigr] \entails  A $\vspace{.5ex}%invalid
\item $A\eor B$, $B\eor C$, $\enot B \entails A \eand C$\vspace{.5ex} %valid
\item $A \eif B$, $\enot A\entails \enot B$ \vspace{.5ex}%invalid
\item $A$, $B\entails \enot(A\eif \enot B)$ \vspace{.5ex}%valid
\item $\enot(A \eand B)$, $A \eor B$, $A \eiff B\entails C$ \vspace{.5ex}%valid 
\end{earg}
...
\noindent\problempart
Determine whether each the following pairs of sentences are tautologically equivalent using complete truth tables. If the two sentences really are tautologically equivalent, write ``equivalent.'' Otherwise write, ``not equivalent.''
\begin{earg}
\item $A$ and $A \eor A$
\item $A$ and $A \eand A$
...
\item $A \eor B$, $A \eif B$, $B \eif A \entails  A \eiff B$  % Valid
\item $A\eor(B\eif A)\entails \enot A \eif \enot B$ %valid
\item $A \eor B$, $A \eif B$, $ B \eif A \entails  A \eand B$ %valid
\item $(B\eand A)\eif C$, $(C\eand A)\eif B\entails (C\eand B)\eif A$ % invalid
\item $\enot (\enot A \eor \enot B)$, $A \eif \enot C \entails  A \eif (B \eif C)$ % invalid.
\item $A \eand (B \eif C)$, $\enot C \eand (\enot B \eif \enot A)\entails C \eand \enot C$ % valid
\item $A \eand B$, $\enot A \eif \enot C$, $B \eif \enot D \entails  A \eor B$ % Invalid
\item $A \eif B\entails (A \eand B) \eor (\enot A \eand \enot B)$ % invalid
\item $\enot A \eif B$,$ \enot B \eif C $,$ \enot C \eif A \entails  \enot A \eif (\enot B \eor \enot C) $% Invalid
...
\label{pr.TT.valid} 
\begin{enumerate}
\item $A\eiff\enot(B\eiff A)\entails A$ % invalid
\item $A\eor B$, $B\eor C$, $\enot A\entails B \eand C$ % invalid
\item $A \eif C$, $E \eif (D \eor B)$, $B \eif \enot D\entails (A \eor C) \eor (B \eif (E \eand D))$ % invalid
\item $A \eor B$, $C \eif A$, $C \eif B\entails A \eif (B \eif C)$ % invalid
\item $A \eif B$, $\enot B \eor A\entails A \eiff B$ % valid
\end{enumerate}
...
\item Suppose that \script{A} is a contradiction. What can you say about the argument \{\script{A}, \script{B}\} $\entails $  \script{C}?
...
\item Suppose that \script{C} is a tautology. What can you say about the argument \{\script{A}, \script{B}\} $\entails $ \script{C}''?
