...
\paragraph{Negation.} For any sentence \metav{A}: If \metav{A} is true, then \enot\metav{A} is false. If \enot\metav{A} is true, then \metav{A} is false. We can summarize this in the \emph{characteristic truth table} for negation:
\begin{center}
\begin{tabular}{c|c}
...
\end{tabular}
\end{center}
The conditional is \emph{asymmetrical}. You cannot swap the antecedent and consequent without changing the meaning of the sentence, because $\metav{A}\eif\metav{B}$ has a very different truth table from $\metav{B}\eif\metav{A}$.
...
Consider the sentence `$(H\eand I)\eif H$'. There are four possible ways to assign True and False to the sentence letter `$H$' and `$I$'---four possible valuations---which we can represent as follows:
\begin{center}
...
The following idea is closely related to that of joint satisfiability:
	\factoidbox{
		The sentences $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ \define{entail} (in TFL) the sentence $\metav{C}$ if there is no valuation of the sentence letters which makes all of $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ true and $\metav{C}$ false.
	}
...
In short, we have a way to test for the validity of English arguments. First, we symbolize them in TFL, as having premises $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$, and conclusion $\metav{C}$. Then we test for entailment in TFL using truth tables. 
...
We are going to use the notion of entailment rather a lot in this book. It will help us, then, to introduce a symbol that abbreviates it. Rather than saying that the TFL sentences $\metav{A}_1, \metav{A}_2, \ldots$ and $\metav{A}_n$ together entail $\metav{C}$, we will abbreviate this by:
...
Note that there is no limit on the number of TFL sentences that can be mentioned before the symbol `$\entails$'. Indeed, we can even consider the limiting case:
	$$\entails \metav{C}$$
This says that there is no valuation which makes all the sentences mentioned on the left hand side of `$\entails$' true whilst making $\metav{C}$ false. Since \emph{no} sentences are mentioned on the left hand side of `$\entails$' in this case, this just means that there is no valuation which makes $\metav{C}$ false. Otherwise put, it says that every valuation makes $\metav{C}$ true. Otherwise put, it says that $\metav{C}$ is a tautology. Equally:
	$$\metav{A} \entails$$
says that $\metav{A}$ is a contradiction.

\section{`$\entails$' versus `$\eif$'}
We now want to compare and contrast `$\entails$' and `$\eif$'. 

Observe: $\metav{A} \entails \metav{C}$ iff there is no valuation of the sentence letters that makes $\metav{A}$ true and $\metav{C}$ false. 

Observe: $\metav{A} \eif \metav{C}$ is a tautology iff there is no valuation of the sentence letters that makes $\metav{A} \eif \metav{C}$ false. Since a conditional is true except when its antecedent is true and its consequent false, $\metav{A} \eif \metav{C}$ is a tautology iff there is no valuation that makes $\metav{A}$ true and $\metav{C}$ false. 
