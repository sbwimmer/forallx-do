...
\open
	...
	\have{hnf}{\exists x (Hx \eand \enot Fx)}\Ei{hanfa}
\close
\have{con}{\exists x (Hx \eand \enot Fx)}\Ee{imp, ga-hnf}
\end{proof}}
...
\end{proof}}
\item $\forall x\exists y Rxy \therefore  \exists y\forall x Rxy$ \hfill \myanswer{Not valid
...
\item $\exists x Mx \eif \exists x Nx$, $\enot \exists x Nx\therefore  \forall x \enot Mx$\hfill \myanswer{Valid\\
\begin{proof}
\hypo{EMiEN}{\exists x Mx \eif \exists x Nx}
\hypo{nEN}{\enot \exists x Nx}
\open
  \hypo{Ma}{Ma}
  \have{EM}{\exists x Mx}\Ei{Ma}
  \have{EN}{\exists x Nx}\ce{EMiEN,EM}
  \have{abs}{\ered}\ri{EN, nEN}
\close
\have{nMa}{\enot Ma}\ni{Ma-abs}
\have{AnM}{\forall x \enot Mx}\Ai{nMa}
\end{proof}}
\end{earg}
...
\end{proof}
\end{multicols}}
NB: the variable `$x$' does not occur in `$Ga$'. When all the quantifiers occur at the beginning of a sentence, that sentence is said to be in \emph{prenex normal form}. Together with the CQ rules, these equivalences are sometimes called \emph{prenexing rules}, since they give us a means for putting any sentence into prenex normal form.
...
\begin{proof}
\hypo{nAa}{\enot \forall x Ax}
  \open
  \hypo{nEna}{\enot \exists x \enot Ax}
    \open
    \hypo{na}{\enot Aa}
    \have{Ena}{\exists x \enot Ax}\Ei{na}
    \have{abs1}{\ered}\ri{Ena,nEna}
    \close
  \have{nna}{\enot \enot Aa}\ni{na-abs1}
  \have{a}{Aa}\dne{nna}
  \have{Aa}{\forall x Ax}\Ai{a}
  \have{abs2}{\ered}\ri{Aa,nAa}
  \close
\have{nnEna}{\enot \enot \exists x \enot Ax}\ni{nEna-abs2}
\have{Ena2}{\exists x \enot Ax}\dne{nnEna}
\end{proof}\\
And here is a proof that does not use any derived rules:
\begin{proof}
	\hypo{nAa}{\enot \forall x Ax} 
	\open
        \hypo{H1}{\exists x \enot Ax}
        \have{H1C}{\exists x \enot Ax \eand \exists x \enot Ax}\ai{H1}
        \have{conc1}{\exists x \enot Ax}\ae{H1C}
	\close
        \open
        \hypo{H2}{\enot \exists x \enot Ax}
          \open
          \hypo{H3}{Ab}
          \have{H3C}{Ab \eand Ab}\ai{H3}
          \have{H3R}{Ab}\ae{H3C}
          \close
          \open
          \hypo{H4}{\enot Ab}
          \have{EH4}{\exists x \enot Ax}\Ei{H4}
          \have{abs4}{\ered}\ri{EH4,H2}
          \have{Ab4}{Ab}\re{abs4}
          \close          
        \have{Ab}{Ab}\tnd{H3-H3R,H4-Ab4}
        \have{Aa}{\forall x Ax}\Ai{Ab}
        \have{abs}{\ered}\ri{Aa,nAa}
        \have{conc2}{\exists x \enot Ax}\re{abs}
        \close
	\have{Ena}{\exists x \enot Ax}\tnd{H1-conc1,H2-conc2}
\end{proof}}
