...
\\\myanswer{If $\metav{A} \proves \metav{B}$, then $(\metav{A} \eand \metav{C})\proves \metav{B}$. After all, if $\metav{A} \proves \metav{B}$, then there is some proof with assumption $\metav{A}$ that ends with $\metav{B}$, and no undischarged assumptions other than $\metav{A}$. Now, if we start a proof with assumption $(\metav{A} \eand \metav{C})$, we can obtain $\metav{A}$ by $\eand$E. We can now copy and paste the original proof of $\metav{B}$ from $\metav{A}$, adding 1 to every line number and line number citation. The result will be a proof of $\metav{B}$ from assumption $\metav{A}$.}
...
\end{fitchproof}
\\This will show that $\metav{A} \proves \metav{B}$. In exactly the same way, we can show that $\metav{B} \proves \metav{A}$. So if we can show that $\metav{A} \eiff \metav{B}$ is a theorem, we can show that $\metav{A}$ and $\metav{B}$ are provably equivalent.}

\myanswer{Conversely, suppose we can show that $\metav{A}$ and $\metav{B}$ are provably equivalent. Then we can prove $\metav{B}$ from the assumption of $\metav{A}$ in $m$ lines, say, and prove $\metav{A}$ from the assumption of $\metav{B}$ in $n$ lines, say. Copying and pasting these proofs together (changing the line numbering where appropriate), we obtain:
\begin{fitchproof}
\open
	\hypo{a}{\metav{A}}
	\have[m]{b1}{\metav{B}}
...
\end{fitchproof}
\\Thus showing that $\metav{A} \eiff \metav{B}$ is a theorem.}

\myanswer{There was nothing special about $\metav{A}$ and $\metav{B}$ in this. So what this shows is that the problem of showing that two sentences are provably equivalent is, essentially, the same problem as showing that a certain kind of sentence (a biconditional) is a theorem.}
